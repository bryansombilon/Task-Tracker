...
const AddTaskModal: React.FC<AddTaskModalProps> = ({ isOpen, onClose, onSubmit, onDelete, initialData, defaultPriority }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/30 dark:bg-black/50 backdrop-blur-md p-4 transition-all">
      <div 
      ...
      >
        ...
        {showDeleteConfirm && (
          <div className="absolute inset-0 z-50 bg-white/95 dark:bg-zinc-900/95 mixed:bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-200">
            ...
            <h3 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 mixed:text-zinc-900 mb-2">Delete Task?</h3>
            <p className="text-zinc-500 dark:text-zinc-400 mixed:text-zinc-500 mb-8 max-w-xs leading-relaxed">
            ...
            <div className="flex gap-3 w-full max-w-xs">
              <button
                ...
                className="flex-1 py-3 px-4 rounded-xl border border-zinc-200 dark:border-zinc-700 mixed:border-zinc-200 text-zinc-600 dark:text-zinc-300 mixed:text-zinc-600 font-bold hover:bg-zinc-50 dark:hover:bg-zinc-800 mixed:hover:bg-zinc-50 transition-colors"
              >
                Cancel
              </button>
              <button
        ...
        <div className="p-6 border-b border-zinc-100 dark:border-zinc-800 mixed:border-zinc-100 flex justify-between items-center bg-zinc-50/50 dark:bg-zinc-800/50 mixed:bg-zinc-50/50 shrink-0">
          <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 mixed:text-zinc-900 flex items-center gap-2">
            {initialData ? (
              <>
                <div className="p-1.5 bg-indigo-100 dark:bg-indigo-900/30 mixed:bg-indigo-100 rounded-lg text-indigo-600 dark:text-indigo-400 mixed:text-indigo-600"><Save size={18} /></div>
                Edit Task
              </>
            ) : (
              <>
                 <div className="p-1.5 bg-indigo-100 dark:bg-indigo-900/30 mixed:bg-indigo-100 rounded-lg text-indigo-600 dark:text-indigo-400 mixed:text-indigo-600"><Plus size={18} /></div>
                 New Task
              </>
            )}
          </h2>
          <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300 mixed:hover:text-zinc-600 transition-colors p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 mixed:hover:bg-zinc-100 rounded-full">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto scrollbar-hide">
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-xs font-bold text-zinc-500 dark:text-zinc-400 mixed:text-zinc-500 uppercase tracking-wide">Task Name</label>
              <button 
                type="button" 
                onClick={handleSmartAnalyze}
                disabled={!formData.name || isAnalyzing}
                className="text-[10px] font-semibold text-indigo-600 dark:text-indigo-400 mixed:text-indigo-600 hover:text-indigo-700 bg-indigo-50 dark:bg-indigo-900/20 mixed:bg-indigo-50 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 mixed:hover:bg-indigo-100 px-2 py-1 rounded-full flex items-center gap-1 transition-colors disabled:opacity-50"
              >
          ...
          {/* ClickUp Link */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-zinc-500 dark:text-zinc-400 mixed:text-zinc-500 uppercase tracking-wide flex items-center gap-1.5">
          ...
          {/* Priority Selection */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-zinc-500 dark:text-zinc-400 mixed:text-zinc-500 uppercase tracking-wide">Priority Level</label>
          ...
          <div className="pt-4 flex gap-3">
            {initialData && onDelete && (
              <button
                ...
                className="p-3 rounded-xl border border-red-200 dark:border-red-800 mixed:border-red-200 text-red-600 dark:text-red-400 mixed:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 mixed:hover:bg-red-50 hover:border-red-300 dark:hover:border-red-700 mixed:hover:border-red-300 transition-colors"
                title="Delete Task"
              >
                <Trash2 size={20} />
              </button>
            ...
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-3 px-4 rounded-xl border border-zinc-200 dark:border-zinc-700 mixed:border-zinc-200 text-zinc-600 dark:text-zinc-300 mixed:text-zinc-600 font-semibold text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800 mixed:hover:bg-zinc-50 transition-colors"
            >
              Cancel
            </button>
            <button
